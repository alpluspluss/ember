...
namespace sprk
{
	...
	void AliasAnalysisPass::detect_memory_issues(const std::vector<std::unique_ptr<SproutNode<> > > &nodes)
	{
		for (const auto &[alloc_node, operations]: memory_operations)
		{
			...
			std::vector<NodeRef> sorted_ops = sort_memory_ops(operations);
	...
	std::vector<NodeRef> AliasAnalysisPass::sort_memory_ops(const std::vector<NodeRef> &operations)
	{
		/* currently assumes the IR is roughly in topological order; which should be anyway */
		/* will need to change if stack reordering is introduced */
	...
	void AliasAnalysisPass::detect_double_free(const NodeRef alloc_node, const std::vector<NodeRef> &ops,
	                                           const std::vector<std::unique_ptr<SproutNode<> > > &nodes)
	{
		auto is_freed = false;
	...
	bool AliasAnalysisPass::points_to_same_memory(const NodeRef ptr1, const NodeRef ptr2) const
	{
		...
		return std::any_of(set1.begin(), set1.end(), [&set2](const NodeRef target)
		{
			return set2.find(target) != set2.end();
		});
	}
			...
			for (size_t i = 0; i < alias_groups.size(); i++)
			{
				...
				for (const NodeRef ptr: group)
				{
					...
					if (ptr < nodes.size() && nodes[ptr])
					{
						...
						if (node->str_id)
							std::cout << " (" << reinterpret_cast<const char *>(node->str_id) << ")";
					}
				}
				std::cout << std::endl;
			}
